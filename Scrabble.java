...
public class Scrabble {
	...
	public static int wordScore(String word) {
		...
		for (int i = 0; i < word.length(); i++){
			...
			score += SCRABBLE_LETTER_VALUES[c - 97];
		}
		score *= word.length();
	...
	public static void playHand(String hand) {
		...
		while (hand.length() > 0) {
			...
			if (input.equals(".") ){
				...
				// Checks if the word is in the hand and if its in the dictionary.
				if (input.length() > hand.length()){
					wordInHand = false;
				}
				else{
					for (int i = 0; i < input.length(); i++){
						if (MyString.countChar(input, input.charAt(i)) > MyString.countChar(hand, input.charAt(i))){
							wordInHand = false;
							break;
						}
					}
				...
				if (isWordInDictionary(input) && wordInHand){
